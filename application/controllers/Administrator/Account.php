...
class Account extends CI_Controller
{
    ...
    public function getBankAccounts()
    {
        $accounts = $this->db->query("
            select 
            ba.*,
            concat_ws('-', ba.account_number, ba.bank_name, ba.account_name) as display_name,
            case ba.status 
            when 1 then 'Active'
            else 'Inactive'
            end as status_text
        ...
        $transactions = $this->db->query("
            select * from(
                select 
                    'a' as sequence,
                    sm.account_id as id,
                    concat('Sales from Bank (', ba.account_number, '-', ba.bank_name, '-', ba.account_name,')') as description,
                    sm.account_id,
                ...
                join tbl_bank_accounts ba on ba.account_id = sm.account_id
                where sm.Status = 'a'
    ...
    public function getCashLedger()
    {
        ...
        $ledger = $this->db->query("
            ...
            select 
                ct.id as id,
                ct.transferDate as date,
                concat('CashTransfer -') as description,
                0.00 as in_amount,
                ct.transferAmount as out_amount
            from tbl_cashtransfer ct
            where ct.Status = 'a'
            and ct.transferFrom = '$this->brunch'
            and ct.transferDate between '$data->fromDate' and '$data->toDate'

            UNION
            select 
                ct.id as id,
                ct.transferDate as date,
                concat('Cash Recieved -') as description,
                ct.transferAmount as in_amount,
                0.00 as out_amount
            from tbl_cashtransfer ct
            where ct.Status = 'a'
            and ct.transferTo = '$this->brunch'
            and ct.transferDate between '$data->fromDate' and '$data->toDate'
    ...
    public function cashTransfer()
    {
        ...
        $data['id'] = '';
        $data['title'] = "Add Cash Transfer";
        $data['content'] = $this->load->view('Administrator/cashtransfer/cashtransfer', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function editCashTransfer($id)
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
        ...
        $data['id'] = $id;
        $data['title'] = "Edit Cash Transfer";
        $data['content'] = $this->load->view('Administrator/cashtransfer/cashtransfer', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function CashTransferList()
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
        ...
        $data['content'] = $this->load->view('Administrator/cashtransfer/cashtransferList', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function recieveCashTransfer()
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
        }
        $data['title'] = "Cash Transfer Recieved";
        $data['content'] = $this->load->view('Administrator/cashtransfer/recievedTransfer', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function getCashTransfer()
    {
        try {
            ...
            if (isset($data->id) && $data->id != '') {
                $clauses .= " AND ct.id = '$data->id'";
            }
            if (isset($data->dateFrom) && $data->dateFrom != '') {
                $clauses .= " AND ct.transferDate between '$data->dateFrom' AND '$data->dateTo'";
            }
            if (isset($data->branchTo) && $data->branchTo != '') {
                $clauses .= " AND ct.transferTo = '$data->branchTo'";
            }
            if (isset($data->branchFrom) && $data->branchFrom != '') {
                $clauses .= " AND ct.transferFrom = '$data->branchFrom'";
            }

            $cashtransfer = $this->db->query("SELECT
                                            ct.*,
                                            btF.Brunch_name AS transferBranchname,
                                            btT.Brunch_name AS reciveBranchname
                                        FROM tbl_cashtransfer ct
                                        LEFT JOIN tbl_brunch btF ON btF.brunch_id = ct.transferFrom
                                        LEFT JOIN tbl_brunch btT ON btT.brunch_id = ct.transferTo
                                        WHERE ct.Status != 'd' $clauses")->result();

            $res = ["status" => true, "message" => $cashtransfer];
    ...
    public function addCashTransfer()
    {
        try {
            ...
            $cashtransfer = array(
                ...
                "Status"         => 'p',
                "AddBy"          => $this->session->userdata('User_Name'),
            );

            $this->db->insert('tbl_cashtransfer', $cashtransfer);

            $res = ["status" => true, "message" => "Cash Transfer Added"];
            echo json_encode($res);
        } catch (\Throwable $th) {
            $res = ["status" => false, "message" => $th->getMessage()];
            echo json_encode($res);
    ...
    public function updateCashTransfer()
    {
        try {
            ...
            $this->db->update('tbl_cashtransfer', $cashtransfer);

            $res = ["status" => true, "message" => "Cash Transfer Updated Success"];
            echo json_encode($res);
        } catch (\Throwable $th) {
            $res = ["status" => false, "message" => $th->getMessage()];
            echo json_encode($res);
    ...
    public function deleteCashTransfer()
    {
        ...
        $this->db->query("UPDATE tbl_cashtransfer SET Status = '$data->Status' WHERE id = '$data->id'");

        $res = "";
        if ($data->Status == 'a') {
            $res = "Approved Cash Transfer";
        } else {
            $res = "Delete Cash Transfer";
        }
        echo json_encode($res);
    }
}
