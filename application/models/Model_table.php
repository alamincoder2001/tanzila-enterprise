...
class Model_Table extends CI_Model{
    ...
    public function getTransactionSummary($date = null){
        $transactionSummary = $this->db->query("
            ...
            (
                select ifnull(sum(ct.transferAmount), 0) from tbl_cashtransfer ct
                where ct.transferTo= " . $this->session->userdata('BRANCHid') . "
                and ct.Status = 'a'
                " . ($date == null ? "" : " and ct.transferDate < '$date'") . "
            ) as cash_recieved,
            (
                select ifnull(sum(ct.transferAmount), 0) from tbl_cashtransfer ct
                where ct.transferFrom= " . $this->session->userdata('BRANCHid') . "
                and ct.Status = 'a'
                " . ($date == null ? "" : " and ct.transferDate < '$date'") . "
            ) as cash_transfer,
            (
            ...
            /* total */
            (
                select received_sales + cash_recieved + received_customer + received_supplier + received_cash + bank_withdraw + loan_received + loan_initial_balance + invest_received + sale_asset
            ) as total_in,
            (
                select paid_purchase + cash_transfer + paid_customer + paid_supplier + paid_cash + bank_deposit + employee_payment + loan_payment + invest_payment + buy_asset
            ) as total_out,
            (
    ...
    public function getBankTransactionSummary($accountId = null, $date = null){
        $bankTransactionSummary = $this->db->query("
            select 
                ...
                (
                    select (received_sales + ba.initial_balance + total_deposit + total_received_from_customer + total_received_from_supplier) - (total_withdraw + total_paid_to_customer + total_paid_to_supplier)
                ) as balance
